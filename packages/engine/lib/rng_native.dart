...
/// cursor — the NATIVE implementation (VM + AOT, where `int` is 64-bit).
///
/// This is the engine's canonical RNG: the same `seed` plus the same `cursor`
/// reproduces the exact output stream, which is what makes runs replayable
/// and the golden tests byte-stable. It relies on 64-bit integer math (Dart's
/// `int` is 64-bit on native and wraps on overflow, exactly what canonical
/// SplitMix64 expects). The web build CANNOT run this (web ints are doubles),
/// so [rng.dart] conditionally swaps in `rng_web.dart` there — a separate,
/// 2^53-safe stream. The selection is at compile time, so this file is never
/// even parsed in a web build.
///
